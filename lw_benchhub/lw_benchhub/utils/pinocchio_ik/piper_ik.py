...
class PiperPinocchioIK:
    ...
    def __init__(self, urdf_path: Optional[str] = None, package_dirs: Optional[list[str]] = None,
        ...
        # Closed-loop policy evaluation does NOT invoke preprocess_device_action,
        # so the casadi-based IK construction is unnecessary. Defer the failure:
        # mark the instance as unusable; any actual solve_* call will then raise.
        if pin is None or RobotWrapper is None:
            self._stub_reason = "pinocchio is required for PiperPinocchioIK. Please install pinocchio>=3.6."
            self._is_stub = True
            return
        if cpin is None or casadi is None:
            self._stub_reason = "casadi and pinocchio.casadi are required to match piper_pinocchio.py logic."
            self._is_stub = True
            return
        self._is_stub = False
    ...
    def reset(self):
        # No-op for stub instances (cmeel pinocchio wheel lacks casadi binding).
        # Eval path doesn't call solve_*, so internal state never gets used.
