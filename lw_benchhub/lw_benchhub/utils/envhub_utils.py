...
def _make_env_cfg(cfg):
    ...
    if "-" in cfg.task:
        ...
        env_cfg = parse_env_cfg(
            ...
            fix_object_pose_cfg=getattr(cfg, "fix_object_pose_cfg", None)
        )
...
def export_env_for_envhub(config_path: str, app_launcher=None):
    ...
    if app_launcher is None:
        ...
        # T3 pipeline path: caller passed an existing AppLauncher; force head-view camera ON.
        # v6 reach-gate / lerobot-eval callers omit app_launcher -> take the branch above,
        # which preserves the original behavior (cfg.enable_cameras unchanged). (Stage4 T3 §5.2)
