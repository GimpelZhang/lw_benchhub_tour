...
def patch_create_teleop_device():
    ...
    # IsaacLab >=2.3.x dropped DEVICE_MAP / RETARGETER_MAP in favor of
    # cfg.class_type / cfg.retargeter_type lookup. The teleop monkey-patch is
    # only needed for interactive teleop data collection; closed-loop policy
    # evaluation does not invoke create_teleop_device. If the legacy symbols
    # are missing, skip the patch entirely instead of crashing at import time.
    try:
        from isaaclab.devices.teleop_device_factory import DEVICE_MAP, RETARGETER_MAP
    except ImportError:
        import logging as _logging
        _logging.getLogger(__name__).info(
            "lw_benchhub.monkey_patch: DEVICE_MAP/RETARGETER_MAP not found "
            "in isaaclab.devices.teleop_device_factory (IsaacLab >=2.3.x). "
            "Skipping patch_create_teleop_device (eval/inference does not use it)."
        )
        return
...
def patch_xform_prim_view_auto_standardize():
    """IsaacLab >=2.3.x requires Xformable prims to have canonical xform op order
    [translate, orient, scale]. USD scenes from lightwheel_sdk (robocasa-libero
    layouts) use [translate, rotateXYZ, scale] for some prims (e.g., table_front_group_1).

    These prims are referenced from a sublayer USD file where we can't author new
    ops (Sdf authoring permission denied), so standardize_xform_ops() fails with
    "Unable to add xform op of type TypeOrient". Workaround: override
    XformPrimView.__init__ to disable validation. The prim is still Xformable and
    downstream code uses GetLocalTransformation() which handles any op order.
    """
    try:
        ...
        _logging.getLogger(__name__).warning(
            f"patch_xform_prim_view_auto_standardize: skipped, IsaacLab not importable yet ({e})"
        )
        return

    _orig_init = XformPrimView.__init__

    def _init(self, prim_path, device="cpu", validate_xform_ops=True,
              sync_usd_on_fabric_write=False, stage=None):
        # Skip the canonical-op-order validation. The xform-op check is a strict
        # invariant for IsaacLab transform writebacks, but the data path used by
        # lerobot-eval reads via GetLocalTransformation which is order-agnostic.
        return _orig_init(self, prim_path, device=device,
                          validate_xform_ops=False,
                          sync_usd_on_fabric_write=sync_usd_on_fabric_write,
                          stage=stage)

    XformPrimView.__init__ = _init


patch_xform_prim_view_auto_standardize()
