...
# lightwheel-sdk >=1.0.x moved ENDPOINT from .loader to .client
try:
...
class LwTaskBase(TaskBase, NoDeepcopyMixin):
    ...
    def __init__(self):
        ...
        _ctx_fix = getattr(self.context, "fix_object_pose_cfg", None)
        if self.fix_object_pose_cfg is None and _ctx_fix is not None:
            self.fix_object_pose_cfg = _ctx_fix
            ...
            for obj_name, obj_placement in self.object_placements.items():
                if obj_name in self.fix_object_pose_cfg:
                    ...
                    if "pos" in self.fix_object_pose_cfg[obj_name]:
                        obj_pos = tuple(self.fix_object_pose_cfg[obj_name]["pos"])
                    if "rot" in self.fix_object_pose_cfg[obj_name]:
                        obj_rot = tuple(self.fix_object_pose_cfg[obj_name]["rot"])
