...
LOG_ROOT="/mnt/robot/stage2_logs"
SCENE_GLOB="/mnt/robot/lw_benchhub/configs/envhub/generated/scene_variation_*.yml"
mkdir -p "$LOG_ROOT"
echo "Stage 2 v6 batch eval started: $(date -Iseconds)" | tee "$LOG_ROOT/run_stage2_all.log"
...
echo "Stage 2 v6 batch eval finished: $(date -Iseconds)" | tee -a "$LOG_ROOT/run_stage2_all.log"
